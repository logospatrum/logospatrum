...
- **`langgraph dev` leaves orphan workers when killed by parent PID only.** The command spawns child processes (uvicorn reloader → uvicorn worker, plus a langgraph_runtime helper); `taskkill /F /PID <listener-pid>` kills only the listening parent and the children survive — they keep the port socket open under the dead PID, and `Get-NetTCPConnection` keeps reporting LISTEN with a non-existent owner until the orphans actually exit (which they don't, because they're now session-leaders with no parent). This is NOT a Windows TIME_WAIT — the socket is live, held by orphans we lost track of. **Always use `taskkill /F /T /PID <pid>` (the `/T` flag walks the process tree)** or, better, start the server via `mcp__Claude_Preview__preview_start` which manages the lifecycle properly. Same applies to `npm run dev` / `next dev`. If the port is already stuck, find stragglers with `Get-CimInstance Win32_Process -Filter "Name='python.exe' OR Name='node.exe'" | Where-Object CommandLine -match 'langgraph|next-server'` and kill them by PID.
