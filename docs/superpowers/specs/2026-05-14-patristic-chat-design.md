...
- Кнопка `💬 спросить` рядом с трудом — кладёт preset-prompt в input чата и закрывает модалку. Шаблон: `«Расскажи о труде "{title}" — {author}. Какие ключевые темы и цитаты?»`. Пользователь может отредактировать перед отправкой. Если активный тред уже есть — добавляется в его input; если нет — создаётся новый.
...
- Preset-промпт «спросить про автора» на уровне строки автора (не только труда).
